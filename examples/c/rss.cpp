...
int main(int argc, char **argv) {
  ...
  fprintf(stdout, "mmaped %d bytes (%dkb, %d pages) at %p\n", ps * NR_PAGES, (ps * NR_PAGES) / 1024, NR_PAGES, map);
  rss(map, ps * NR_PAGES);
  prompt();
  ...
  if (mprotect(&map[ps * NR_PAGES - 2 * ps], ps * 2, PROT_READ | PROT_WRITE | PROT_EXEC) == -1) err(1, "mprotect");
  fprintf(stdout, "mprotected two page at %p to PROT_READ | PROT_WRITE | PROT_EXEC\n", &map[ps * NR_PAGES - 2 * ps]);
  rss(map, ps * NR_PAGES);
  prompt();
